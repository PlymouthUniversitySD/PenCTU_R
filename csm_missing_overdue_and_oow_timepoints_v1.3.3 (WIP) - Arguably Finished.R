...
missing_overdue_and_oow_timepoints <- function(data, metadata, by_site = FALSE, include_split_CSVs = FALSE) {
  ...
  participant_count <- mutate(participant_count, count = row(data_unique_IDs))
  ...
  #Define participant count by site for use in graph plot IF split by site.
  site_participant_count <- sites_by_unique_IDs
  site_participant_count <- group_by(site_participant_count, redcap_data_access_group)
  site_participant_count <- mutate(site_participant_count, count = row_number())
  site_participant_count <- ungroup(site_participant_count)
  site_participant_count <- dplyr::select(site_participant_count, count)
...
# # # # # # # # # # # ############################################EXCLUSIVELY FOR OOW FUNCTION###############################oow#oow#oow#
        ...
        #Adds a column corresponding to the participant count by site for plotting purposes.
        dfIDevent <- add_column(dfIDevent, participant_count_by_site = site_participant_count[[yi,1]])
        #Transform the column's values to be numeric for future functions.
...
# # # # # # # # # # # ############################################EXCLUSIVELY FOR OOW FUNCTION###############################oow#oow#oow#
  ...
  timepoint_plot <- function (metadata, dated_timepoints) {
...
#  dated_timepoints <- dplyr::
  ...
  if (by_site == TRUE) {
    ...
    ymin <- min(dated_timepoints["participant_count_by_site"],0)
    ymax <- max(dated_timepoints["participant_count_by_site"])
  ...
  y_breaks <- seq(from = ymin, to = ymax+5, by = 2)
  ...
  if (by_site == TRUE) {
    ggplot(data = dated_timepoints, 
      ...
      geom_point(data = dated_timepoints, 
                 ...
                 size = 2, 
                 alpha = 0.8) + 
      facet_grid(redcap_data_access_group ~ ., scales = "free_y") +
      #Set X-axis breaks
